...
wind_farm_df = cfd_master_list[cfd_master_list['Technology'].str.contains('Wind')][['CFD ID', 'Technology']]
...
combined_df = wind_speed.merge(wind_farm_df, on='CFD ID', how='left')

# COMMAND ----------

# Create a pivot table
wind_speed_pivot = pd.pivot_table(combined_df, values='Wind Speed', index=['Times'], columns=['CFD ID', 'Technology'], aggfunc='first')
...
offshore_columns = [col for col in wind_speed_pivot.columns if col[1] == 'Offshore Wind']
onshore_columns = [col for col in wind_speed_pivot.columns if col[1] == 'Onshore Wind']

# Calculate row-wise average for Offshore Wind and Onshore Wind
wind_speed_pivot['Offshore Wind Average'] = wind_speed_pivot[offshore_columns].mean(axis=1)
wind_speed_pivot['Onshore Wind Average'] = wind_speed_pivot[onshore_columns].mean(axis=1)
...
if isinstance(wind_speed_pivot.columns, pd.MultiIndex):
    wind_speed_pivot.columns = wind_speed_pivot.columns.droplevel(1)
else:
    # If the columns are tuples, extract only the CFD ID part
    wind_speed_pivot.columns = [col[0] if isinstance(col, tuple) else col for col in wind_speed_pivot.columns]
...
wind_speed_pivot.to_csv('/dbfs/mnt/dw-silver/01-cfd/ELFO/inputs/Wind/Yearly Load Factors/Wind Streams.csv', index=False)

# COMMAND ----------

os.makedirs('/dbfs/mnt/dw-silver/01-cfd/ELFO/inputs/Wind/Yearly Load Factors/', exist_ok=True)
wind_speed_pivot.to_parquet('/dbfs/mnt/dw-silver/01-cfd/ELFO/inputs/Wind/Yearly Load Factors/Wind Streams.parquet', index=False)
