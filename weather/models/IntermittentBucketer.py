...
class IntermittentBucketer(Bucketer):
	...
	def get_Scaling_Structure(self):
		scaling_structure = {category: {n: [] for n in range(0, len(self.bucket_Definition))} for category in range(self.historical_End_Date.year - self.historical_Start_Date.year + 1)}
		...
		for category in self.bucket_Structure:
			for bucket in self.bucket_Structure[category]:
				for date in self.bucket_Structure[category][bucket]:
					scaling_structure[date.year - self.historical_Start_Date.year][bucket].append(date)
