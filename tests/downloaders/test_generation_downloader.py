...
@pytest.fixture
def normal_day_generation_df() -> pd.DataFrame:
    """Generate normal day generation data (48 settlement periods)."""
    # Normal day: 48 settlement periods with realistic varying quantities
    quantities = [
        19.104,
        19.171,
        19.174,
...
@pytest.fixture
def spring_forward_generation_df() -> pd.DataFrame:
    """Generate spring forward day generation data (46 settlement periods).

    Spring forward data has 46 periods (1-46) with realistic quantities.
    """
    quantities = [
        0.156,
        0.056,
    ...
    data = []
    for period in range(1, 47):  # periods 1-46
        data.append(
            {
                "settlementDate": "2023-03-26",
                "settlementPeriod": period,
                "bmUnit": "C__PSTAT011",
                "quantity": quantities[period - 1],
            }
...
@pytest.fixture
def fall_back_generation_df() -> pd.DataFrame:
    """Generate fall back day generation data (50 settlement periods).

    Fall back data has 50 periods with mostly 0.0 quantities until period 37.
    """
    # First 36 periods are 0.0, then realistic quantities for periods 37-50
    quantities = [0.0] * 36 + [
        11.386,
        14.522,
        14.602,
    ...
    data = []
    for period in range(1, 51):  # periods 1-50
        data.append(
            {
                "settlementDate": "2023-10-29",
                "settlementPeriod": period,
                "bmUnit": "C__PSTAT011",
                "quantity": quantities[period - 1],
            }
...
class TestGenerationDataDownloader:
    ...
    def test_download_normal_day_timezone_handling(
        ...
        with (
            ...
            raw_utc_times = downloader_with_temp_dir._create_hourly_utc_datetime(
                normal_day_generation_df.rename(
                    columns={
            ...
            assert len(raw_unique) == 24, (
                f"Raw periods should map to 24 unique hours, got {len(raw_unique)}"
            )
    ...
    def test_download_spring_forward_timezone_handling(
        ...
        with (
            ...
            # Test raw settlement period mapping (before aggregation)
            raw_utc_times = downloader_with_temp_dir._create_hourly_utc_datetime(
                spring_forward_generation_df.rename(
                    columns={
                        "settlementDate": "settlement_date",
                        "settlementPeriod": "settlement_period",
                    }
            ...
            raw_unique = sorted(raw_utc_times.dt.floor("h").unique())
            assert len(raw_unique) == 23, (
                f"Raw periods should map to 23 unique hours, got {len(raw_unique)}"
            )
    ...
    def test_download_fall_back_timezone_handling(
        ...
        with (
            ...
            assert len(time_hours) == 25, (
                f"Should have 25 unique hour records, got {len(time_hours)}"
            )

            # Test raw settlement period mapping (before aggregation)
            raw_utc_times = downloader_with_temp_dir._create_hourly_utc_datetime(
                fall_back_generation_df.rename(
                    columns={
                        "settlementDate": "settlement_date",
                        "settlementPeriod": "settlement_period",
                    }
            ...
            raw_unique = sorted(raw_utc_times.dt.floor("h").unique())
            assert len(raw_unique) == 25, (
                f"Raw periods should map to 25 unique hours, got {len(raw_unique)}"
            )
            ...
            assert raw_end == expected_end, f"Raw data should end at {expected_end}, got {raw_end}"
    ...
    def test_large_dataset_aggregation_efficiency_regression(
        ...
        with (
            ...
            assert na_count == 0, (
                f"All datetime values should be valid, found {na_count} NaT values"
            )


if __name__ == "__main__":
    # Allow running tests directly with python -m pytest tests/downloaders/test_generation_downloader.py
