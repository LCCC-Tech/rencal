...
def fun(language):
    import_module = __import__('lang.' + language)
    return True
```
...
```python "Individual terms" /Even.*supported/
...
```python "return true;" ins="inserted" del="deleted"
def demo():
    print('These are inserted and deleted marker types');
...
```diff lang="python"
  def process_data():
    # This entire block gets highlighted as Python,
    # and we can still add diff markers to it!
-   print('Old data processing code to be removed')
+   print('New and optimized data processing!')
```
...
```python title="balances.py"
MARKET_DATE = datetime.today() - timedelta(days=1)
START_DATE = MARKET_DATE - timedelta(days=30)

db = DatabaseDelta(data_date=MARKET_DATE, market_date=MARKET_DATE, dataset_name='Default')
bank_balances = db.get_Bank_Balances(start_date=START_DATE)
bank_balances = bank_balances.toPandas()

print(bank_balances.head())
```
...
<VideoAside title="Watch a video about LCCC" href="https://www.youtube.com/watch?v=L6eWBXTkR10" />
