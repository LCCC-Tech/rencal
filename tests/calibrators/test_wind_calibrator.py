...
class TestWindCalibrator:
    ...
    def test_wind_calibrator_default_init(self, default_calibrator):
        ...
        assert np.array_equal(sorted(default_calibrator.plants.data["technology"].unique()), ["Offshore Wind", "Onshore Wind"]) is True
    ...
    def test_wind_calibrator_default_extract_resource_timeseries_for_plants(self, default_calibrator):
        ...
        assert len(resources_for_plants) == len(default_calibrator.resource.data.time) * len(default_calibrator.plants.data)
    ...
    def test_wind_calibrator_default_get_plant_generation_temporal_bounds(self, default_calibrator):
        ...
        sample_gen_data = pd.DataFrame({
            ...
            "time": [datetime.strptime(dt, "%Y-%m-%d %H:%M:%S") for dt in ["2000-12-12 01:00:00", "2000-12-12 02:00:00", "2000-12-12 03:00:00"]] * 3,
        ...
        assert default_calibrator._get_plant_generation_temporal_bounds(sample_gen_data).equals(sample_bounds)
    ...
    def test_wind_calibrator_default_fit_weibull_dist_to_plant(self, default_calibrator):
        ...
        assert default_calibrator._fit_weibull_dist_to_plant(("B", np.array([1.0, 2.0, 3.0], dtype=np.float32))) == ("B", pytest.approx(2.738554), pytest.approx(2.258588))
...
class TestWindCalibratorIntegration:
    ...
    def test_calibration(self, temp_output_dir):
        ...
        calibrator = WindCalibrator(data_path=Path(__file__).parents[1] / "data", output_path=temp_output_dir, visual_output=True)
