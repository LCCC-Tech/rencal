...
class VersionedColumnsNDArray:
    ...
    def attach_data(self, data: NDArray[np.floating]) -> "VersionedColumnsNDArray":
        ...
        if data.shape[1] < self.n_raw_features:
            raise ValueError(
                f"Data has fewer columns than n_raw_features ({self.n_raw_features}): {data.shape[1]} columns provided"
            )
...
class BucketedData:
    ...
    def get_histogram(
        ...
        for column in range(len(self.mask)):
            ...
            histogram[column] = np.bincount(column_bin_index, minlength=number_of_bins).astype(
                np.uint32
            )

        return histogram

    def get_fast_histogram(
        self,
        ...
        if last_full_block >= first_full_block:
            ...
            historical_distributions = (
                prefix_histograms[last_full_block, self.mask, :]
                - prefix_histograms[first_full_block - 1, self.mask, :]
            )
        else:
    ...
    def append_resampled_columns(
        ...
        if self.new_cdf.shape != (n_features_to_modify, n_bins):
            raise ValueError(
                f"new_cdf must have shape ({n_features_to_modify}, {n_bins}), but has shape {self.new_cdf.shape}"
            )
        if len(self.duplicate_histogram_positions) != n_features_to_modify:
            raise ValueError(
                "duplicate_histogram_positions must have one entry per duplicated feature. "
        ...
        for j, col in enumerate(self.duplicate_canonical_streams):
            ...
            bin_classes = position_proxy.astype(
                np.int32
            )  # Cannot use unsigned as we need to access bin_classes - 1 un-lazily in computing observations_before;
            ...
            observations_before = np.where(
                bin_classes > 0, self.old_cdf[prev_hist_j, bin_classes - 1], 0
            )
            observations_within = self.old_pdf[prev_hist_j, bin_classes]
            percentile_aprox = (
                observations_before + fractional_part * observations_within
            ) / old_total[prev_hist_j]
            ...
            resampled = ((new_bin_classes + rng.random(n_active)) * bin_width).astype(
    ...
    def compute_optimized_distributions(
        ...
        if observations_per_column.ndim == 0:
            observations_per_column = np.full(
                (n_streams,), observations_per_column.item(), dtype=np.int32
            )
        ...
        lambda_guess = np.zeros(
            (n_streams,), dtype=np.float64
        )  # start at zero, replicating initial distribution
        ...
        for _ in range(max_iter):
            ...
            good = (
                var > 1e-18
            )  # avoid divide by near-zero for low-variance columns, will take bisection step instead for those
