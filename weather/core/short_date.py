...
class ShortDate:
    ...
    def add_with_context(self, timedelta, context_year):
        ...
        """
        addition = (
            self.to_datetime(context_year) + timedelta
        )  # Ideally, "timedelta" is a datetime.timedelta object expressed in integer days
    ...
    def subtract_with_context(self, timedelta, context_year):
        ...
        subtraction = (
            self.to_datetime(context_year) - timedelta
        )  # Here timedelta can also be a datetime.datetime,
    ...
    def __gt__(self, date):
        ...
        """
        return datetime.datetime(LEAP_YEAR, self.month, self.day) > datetime.datetime(
            LEAP_YEAR, date.month, date.day
        )
...
MONTHLY_BUCKET_DEFINITION = [
    ShortDate.from_list([1, 1]),
    ShortDate.from_list([2, 1]),
    ShortDate.from_list([3, 1]),
    ShortDate.from_list([4, 1]),
    ShortDate.from_list([5, 1]),
    ShortDate.from_list([6, 1]),
    ShortDate.from_list([7, 1]),
    ShortDate.from_list([8, 1]),
    ShortDate.from_list([9, 1]),
    ShortDate.from_list([10, 1]),
    ShortDate.from_list([11, 1]),
    ShortDate.from_list([12, 1]),
]
WEEKLY_BUCKET_DEFINITION = [
    ShortDate.from_list([1, 1]),
    ShortDate.from_list([1, 8]),
    ShortDate.from_list([1, 15]),
    ShortDate.from_list([1, 22]),
    ShortDate.from_list([1, 29]),
    ShortDate.from_list([2, 5]),
    ShortDate.from_list([2, 12]),
    ShortDate.from_list([2, 19]),
    ShortDate.from_list([2, 26]),
    ShortDate.from_list([3, 5]),
    ShortDate.from_list([3, 12]),
    ShortDate.from_list([3, 19]),
    ShortDate.from_list([3, 26]),
    ShortDate.from_list([4, 2]),
    ShortDate.from_list([4, 9]),
    ShortDate.from_list([4, 16]),
    ShortDate.from_list([4, 23]),
    ShortDate.from_list([4, 30]),
    ShortDate.from_list([5, 7]),
    ShortDate.from_list([5, 14]),
    ShortDate.from_list([5, 21]),
    ShortDate.from_list([5, 28]),
    ShortDate.from_list([6, 4]),
    ShortDate.from_list([6, 11]),
    ShortDate.from_list([6, 18]),
    ShortDate.from_list([6, 25]),
    ShortDate.from_list([7, 2]),
    ShortDate.from_list([7, 9]),
    ShortDate.from_list([7, 16]),
    ShortDate.from_list([7, 23]),
    ShortDate.from_list([7, 30]),
    ShortDate.from_list([8, 6]),
    ShortDate.from_list([8, 13]),
    ShortDate.from_list([8, 20]),
    ShortDate.from_list([8, 27]),
    ShortDate.from_list([9, 3]),
    ShortDate.from_list([9, 10]),
    ShortDate.from_list([9, 17]),
    ShortDate.from_list([9, 24]),
    ShortDate.from_list([10, 1]),
    ShortDate.from_list([10, 8]),
    ShortDate.from_list([10, 15]),
    ShortDate.from_list([10, 22]),
    ShortDate.from_list([10, 29]),
    ShortDate.from_list([11, 5]),
    ShortDate.from_list([11, 12]),
    ShortDate.from_list([11, 19]),
    ShortDate.from_list([11, 26]),
    ShortDate.from_list([12, 3]),
    ShortDate.from_list([12, 10]),
    ShortDate.from_list([12, 17]),
    ShortDate.from_list([12, 24]),
]
